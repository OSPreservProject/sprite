if (!defined &_PROCUSER) {
    ...
    eval 'sub PROC_EVICTING {0x20000;}';
    eval 'sub PROC_KILLING {0x40000;}';
    if (!defined &_ASM) {
	require 'sprite.ph';
	require 'spriteTime.ph';
	if (defined &KERNEL) {
	    require 'sigTypes.ph';
	    require 'machTypes.ph';
	    require 'user/vmTypes.ph';
	}
	else {
	    require 'kernel/sigTypes.ph';
	    require 'kernel/machTypes.ph';
	    require 'vmTypes.ph';
	}
    }
    if (!defined &_ASM) {
	...
	eval 'sub PROC_MAX_ENVIRON_NAME_LENGTH {4096;}';
	eval 'sub PROC_MAX_ENVIRON_VALUE_LENGTH {4096;}';
