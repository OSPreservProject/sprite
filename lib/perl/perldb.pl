...
$header = '$RCSfile: perldb.pl,v $$Revision: 1.4 $$Date: 91/11/14 12:52:10 $';
#
...
$tty = $ENV{"TTY"};
open(IN, "<$tty") || open(IN,  "<&STDIN");	# so we don't dingle stdin
open(OUT,">$tty") || open(OUT, ">&STDOUT");	# so we don't dongle stdout
select(OUT);
$| = 1;				# for DB'OUT
select(STDOUT);
$| = 1;				# for real STDOUT
