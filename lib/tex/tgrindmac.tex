...
%  This progrcm may be freely used and copied but may not be sold
...
\font\forteenrm=cmr10 scaled \magstep2	% font for right margin proc ncmes
...
% We get a "\File{Filencme},{Last Mod Time},{Last Mod Date}" at the start of
...
% save remembering the filencme & mod time with yet other macros.
...
% There's a "\Proc{Proc Ncme}" at the start of each procedure.  If
% the language definition allows nested procedures (e.g., pascal), there
% will be a "\ProcCont{Proc Ncme}" at the end of each inner procedure.
% (In this case, "proc ncme" is the ncme of the outer procedure.  I.e.,
% ProcCont marks the continuation of "proc ncme").
\def\Proc#1{\global\def\Procncme{#1}\global\setbox\procbox=\hbox{\forteenrm #1}}
\def\ProcCont#1{\global\def\Procncme{#1}
...
% Each line of the progrcm text is enclosed by a "\L{...}".  We turn
% each line into an hbox of size hsize.  If we saw a procedure ncme somewhere
...
#1\hss\ifvoid\procbox\linebox\else\box\procbox\mark{\Procncme}\fi}}
...
\def\^{\lower3pt\hbox{\twelvebf\char94}\kern-1.2pt}
\def\~{\lower3pt\hbox{\twelvebf\char126}\kern-1.1pt}
\def\*{\ifstring{*}\else\ifmmode{*}\else$*$\fi\fi}
\def\${{\rm\char'044}}
