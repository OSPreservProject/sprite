...
%%
\def\@rm{h-med} % the font used for \rm
\def\@it{h-obl} % the font used for \it
\def\@bf{h-bol} % the font used for \bf
\def\@sl{h-obl} % the font used for \sl
\def\@tt{c-con} % the font used for \tt
\def\@sf{t-rom} % the font used for \sf
\def\@sc{h-sc}%the font used for \sc
\def\@sy{s-med} % the symbols font
\def\@ms{s-obl} % slanted symbols (for Greek)

% this is invoked by \sf
\def\@dosfchars{\let\pcopyright\@copyrightTR
  \let\pregistered\@registeredTR \let\ptrademark\@trademarkTR}

% this is invoked by \rm, \it, etc (except \sf)
\def\@dononsf{\let\pcopyright\@copyrightH
  \let\pregistered\@registeredH \let\ptrademark\@trademarkH}

\def\PSfontversion{Helvetica}
