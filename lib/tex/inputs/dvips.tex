...
You can also specify a different input encoding with \.{-i}.  This
...
can only be accessed through such an `input' reencoding.  Any ligature
or kern information specified in the input encoding is ignored by
\.{afm2tfm}.
...
this reason, the file can be used as an input or output encoding file
...
In any case, you must give appropriate name of the font that
...
If you change the input encoding to a font, you must specify the
...
\.{-i {\it file}}
This specifies a file to use for the input encoding of the
...
the input and target output encodings of the font.

\.{-o {\it file}}
This specifies a file to use for the target output encoding of the
...
If you are using a {\tt pfb} file that is also has a different input
...
it on input with {\tt MyEncoding} stored in {\tt myenc.enc}, a line
such as
\vskip\baselineskip
...
{\vskip0pt\parskip=0pt\begverb{`\\}
 ...
 c # Uncollated copies              x # Override dvi magnification
 ...
 r*  Reverse order of pages         X # Horizontal resolution
 s*  Enclose output in save/restore Y # Vertical resolution
...
Due to the universality of buggy PostScript previewers and spoolers, this
is enabled by default; use \.{-K0} to turn it off.
...
\^{{\tt pk}}
in the path.  A {\tt\%d} is replaced by the resolution at which the
font is desired; a {\tt\%f} is replaced by the base file name of the
font (such as {\tt cmr10}); a {\tt\%m} is replaced by the \MF\ mode
for this font; a {\tt\%p} is replaced by {\tt pk}; and finally,
{\tt\%\%} is replaced by a single literal {\tt\%}.
...
experiences and comments are welcome.  It was added by Jim Hafner, IBM
Research, {\tt hafner@almaden.ibm.com}, so please direct your comments
there.  Besides the changes to the code itself, there are additional
\TeX\ macro files \.{dpscolor.tex} and \.{dpsblack.tex}.  There are also
...
All the color macro commands are defined in \.{dpscolor.tex} (or
\.{dpscolor.sty}).  To access these macros simply add to the top of your
\TeX\ file the command
\cmd{\ttbackslash input dpscolor}
\noindent
or, if your document uses style files like La\TeX, add the \.{dpscolor}
style option as in 
\cmd{\ttbackslash documentstyle[12pt,dpscolor]\char123article\char125}
...
can browse the file \.{dpscolor.tex} for a list of the predefined colors.
...
call to input \.{dpscolor.tex} with \.{dpsblack.tex} (and similarly for the
\.{.sty} files).  So in the above example, replacing the word
\.{dpscolor} with \.{dpsblack} suffices.  This file defines the color macros
...
\.{Goldenrod} (approximate PANTONE 109) for your device
...
\cmd{userdict begin \\
/Goldenrod \char123\ 0 0.10 0.75 0.03 \char125\ bind def}
\noindent
...
This mechanism, together with additions to \.{dpscolor.tex} and
\.{dpsblack.tex} (and the \.{.sty} files), can also be used to predefine
