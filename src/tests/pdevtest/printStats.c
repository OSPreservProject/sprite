    ...
    Time *timePtr;
{
	...
	fprintf(stream, "Idle ticks %0.0f/%d = %6.2f%% Idle, Context Sw. %d inv %d full %d, Elapsed time ",
	       ...
	       (double)lowTicks/
		   (double)endSchedPtr->processor[0].idleTicksPerSecond * 100.,
	       endSchedPtr->processor[0].numContextSwitches -
		   startSchedPtr->processor[0].numContextSwitches,
	       endSchedPtr->processor[0].numInvoluntarySwitches -
		   startSchedPtr->processor[0].numInvoluntarySwitches,
	       endSchedPtr->processor[0].numFullCS -
		   startSchedPtr->processor[0].numFullCS);
    }
