     ...
     int from_tty;
{
  ...
  if (buffer >= 0 && buffer < statbuf.st_size)
   ...
   {
  /*
   * Sprite limits stack growth to 2megs at a time.  Touch the buffer at
   * 1 meg increments to grow stack.
   */
  int i;
  for (i = buffer-1; i > 0; i -= 1024*1024)
        stringtab[i] = 0;
  }
#endif
#endif
...
#ifdef sprite
		/*
		 * Sprite libraries may end in both .o and .go.
		 */
     ...
     struct partial_symtab *pst;
{
   ...
   */
  int i;
  for (i = stsize-1; i > 0; i -= 1024*1024)
        stringtab[i] = 0;
  }
#endif
#endif
