...
validate_files ()
{
  if (execfile != 0 && corefile != 0)
	...
	return;
	 ...
	 /*
	  * The core_aouthdr in Sprite as generated by gcore is 
	  * only a guess at the real exec header. The only thing that
	  * should always be right is the text size.
	  */
	  && (core_aouthdr.a_text != exec_aouthdr.a_text))
#else
