...
#define	O(f)	((int)&(((Mach_RegState *) 0)->f)/sizeof(int))
  if (r < MACH_NUM_GPRS) return O(regs[r]);
  if (r < PC_REGNUM) return  O(fpRegs[r-MACH_NUM_GPRS]);
     ...
     char *regs;
{
  ...
  Kdbx_Trace(DBG_GET_STOP_INFO, (char *) 0, (char *)&stopInfo,
               sizeof(stopInfo));
  bcopy(stopInfo.regs.regs,regs,4*MACH_NUM_GPRS);
  bcopy(stopInfo.regs.fpRegs,regs+4*MACH_NUM_GPRS,4*MACH_NUM_FPRS);
     ...
     int	regno;
{
  ...
  ERROR_NO_ATTACHED_HOST;
    if (regno < 0) { 
	for (i = 0; i < MACH_NUM_GPRS + MACH_NUM_FPRS; i++) {
