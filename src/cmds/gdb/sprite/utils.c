...
#ifdef sprite
/*
 * Cached value for ISATTY((stdout)). This value is cached on Sprite
 * because the isatty() ioctl requires a pdev exchange with the tty driver
 * taking at least 2 ms.  This makes output very slow on gdb.
 */
int isatty_stdout;
#endif
...
#ifdef sprite
  if (stream != stdout || !isatty_stdout || lines_per_page == 0)
#else  
