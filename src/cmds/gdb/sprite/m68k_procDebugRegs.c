...

{
	...
	regs->r_sr = regStatePtr->statusReg;
	bcopy ((char *)&(regStatePtr->regs), (char *) &(regs->r_dreg),
...

{
	...
	regStatePtr->statusReg = regs->r_sr;
	bcopy ((char *) &(regs->r_dreg), (char *)&(regStatePtr->regs), 
					MACH_NUM_GPRS*sizeof(int));
}
