...
/*
 * To allow cross machine type debugging in Sprite we bcopy the 
 * data before returning it.  This is done so we can debugger sun3
 * from sun4 that have stricter alignment constraints.
 */
...
#ifdef KGDB
  union { 
   double	foo; /* double forces alignment. */
   char	buffer[8];
  } tmpMem;
  bcopy(valaddr,tmpMem.buffer,8);
  valaddr = tmpMem.buffer;
#endif
