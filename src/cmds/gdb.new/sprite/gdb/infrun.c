     ...
     char **env;
{
  ...
  len = 5 + 6 + strlen (exec_file) + 1 + strlen (allargs) + 1 + /*slop*/ 10;
  ...
  shell_command[0] = '\0';
#endif
  strcat (shell_command, "exec debug ");
...
#ifdef sprite
      /*
       * This appears to be needed on Sprite.
       */
...
#ifdef sprite
    /*
     * Since Sprite pids are displayed in hex the atoi doesn't work. We
     * allow the attach command to use any C expression.
     */
    pid = value_as_long (evaluate_expression (parse_c_expression (args)));
#else
  pid = atoi (args);
#endif /* sprite */
