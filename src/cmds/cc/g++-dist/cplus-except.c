...
/* Cache `setjmp', `longjmp', and `unhandled_exception' after
   default conversion.  Maybe later they will get built-in.  */
static tree BISJ, BILJ, BIUE;
...
call_to_unhandled_exception ()
{
  ...
  tree parms = tree_cons (NULL_TREE,
			  combine_strings (build_string (strlen (input_filename), input_filename)),
...
void
expand_cplus_end_catch (for_reraise)
     int for_reraise;
{
  if (current_exception_type
      && TYPE_NEEDS_DESTRUCTOR (current_exception_type))
     ...
     tree exceptions;
{
  ...
  if (exceptions == NULL_TREE)
      ...
      if (in_try_block (1) == 0)
	{
      ...
      expand_assignment (build (NOP_EXPR, ptr_type_node,
  ...
  current_exception_ptr = build (NOP_EXPR, NULL_TREE,
				 TREE_OPERAND (current_exception_object, 0));
