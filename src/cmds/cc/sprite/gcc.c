...
/*
 * Generate tables containing specs for all known target machines:
 */
...
/* Default prefixes to attach to command names.  If a name contains a
    "%s", then it will be replaced with the type of machine on which
    we're running, to get the correct binaries for this machine. */

char *standard_exec_prefix = "/sprite/lib/gcc/%s.md/";
char *standard_exec_prefix_1 = "/sprite/cmds.%s/";

char *standard_startfile_prefix = "/sprite/lib/%s.md/";
char *standard_startfile_prefix_1 = "/sprite/lib/gcc/%s.md/";

/* Type of machine on which we're running (used to select correct
    binaries for programs like cpp). */

char *machine;
int machine_length;		/* No. of characters in machine. */
     ...
     char *prog;
{
  ...
  size += strlen (prog) + machine_length + 1;
  if (machine_suffix)
  ...
  if (!win && env_exec_prefix)
    {
      sprintf(temp, env_exec_prefix, machine);
      strcat (temp, prog);
      win = (access (temp, X_OK) == 0);
    }
...
/*** The rest just process a certain constant string as a spec.  */
          ...
          case '+':
	    do_spec_1 (target->cplus1_spec, 0);
	    break;

	  case '1':
	    do_spec_1 (target->cc1_spec, 0);
	    break;
	  ...
	  case 'c':
	    do_spec_1 (target->signed_char_spec, 0);
	    break;

	  case 'C':
	    do_spec_1 (target->cpp_spec, 0);
	    break;
	  ...
	  case 'm':
	    obstack_grow (&obstack, target_name, strlen (target_name));
	    arg_going = 1;
	    break;
	  ...
	  case 'P':
	    {
	      char *x = (char *) alloca(strlen(target->cpp_predefines) * 2 + 1);
	      char *buf = x;
	      char *y = target->cpp_predefines;

	      /* Copy all of target->cpp_predefines into BUF,
     ...
     char *name;
{
  ...
  if (!win && env_exec_prefix)
    {
      sprintf (temp, env_exec_prefix, target_name);
      strcat (temp, name);
      win = (access (temp, R_OK) == 0);
    }
  ...
  if (!win)
    {
      sprintf (temp, standard_startfile_prefix, target_name);
      strcat (temp, name);
      win = (access (temp, R_OK) == 0);
    }
  ...
  if (!win)
    {
      sprintf (temp, standard_startfile_prefix_1, target_name);
      strcat (temp, name);
      win = (access (temp, R_OK) == 0);
    }
     ...
     char **argv;
{
  ...
  programname = argv[0];
  if ((basename = strrchr(programname, '/')) != NULL) {
      ++basename;
  } else {
      basename = programname;
  }
  if (strcmp(basename, "g++") == 0) {
      cplusplus = 1;
  }
  ...
  if (vflag)
    {
      ...
      fprintf (stderr, "target machine is %s\n", target_name);      
      if (n_infiles == 0)
	exit (0);
    }
  ...
  for (i = 0; i < n_infiles; i++)
    {
      ...
      for (cp = (cplusplus ? target->cplusplus_specs : target->base_specs);
           cp->spec; cp++)
	{
...
validate_all_switches ()
{
  ...
  for (comp = (cplusplus ? target->cplusplus_specs : target->base_specs);
       comp->spec; comp++)
    {
      p = comp->spec;
      while (c = *p++)
