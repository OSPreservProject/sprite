					 ...
					 * server is bailing out */
{
    ...
    if (sendReply) {
	if (status > 200) {
	    syslog(LOG_DEBUG,"PdevCleanup: status %d (0x%x)\n",
		   status, status);
	}
					 ...
					 * request ready for processing. */
{
	...
	switch (requestPtr->hdr.operation) {
...
#endif /* lint */
		if (status > 200) {
		    syslog(LOG_DEBUG,"PdevServReq (open): status %d (0x%x)\n",
			   status, status);
		}
		...
		if (status == SUCCESS) {
		    ...
		    if (status > 200) {
			syslog(LOG_DEBUG,"PdevServReq (Ioctl): status %d (0x%x)\n",
			       status, status);
		    }
...
#endif /* lint */
		if (status > 200) {
		    syslog(LOG_DEBUG,"PdevServReq (get_attr): status %d (0x%x)\n",
			   status, status);
		}
		ReplyNoData(srvPtr, status, reply.selectBits);
		break;
