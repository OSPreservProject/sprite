 ...
 /* Create a call sequence customized for this function
   ...
   if (remote_debugging) {
	...
	for (i = 0; i < nargs; i++) {
            ...
            int len = TYPE_LENGTH (VALUE_TYPE (arg));
	       if (ap + len > &(argBuffer[512]) )
		    break;
	       bcopy(VALUE_CONTENTS (arg),ap,len);
	       ap += len;
	}
	val = call_remote_function(funaddr,nargs,ap - argBuffer,argBuffer);
	return value_from_longest (builtin_type_int, val);
   }
#endif	
