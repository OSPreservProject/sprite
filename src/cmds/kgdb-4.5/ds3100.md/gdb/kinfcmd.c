...
static void
step_1 PARAMS ((int, int, char *, int));
...
extern int trap_expected_after_continue;
...
nexti_command (count_string, from_tty)
     ...
     int from_tty;
{
  step_1 (1, 1, count_string, 0);
}

/* ARGSUSED */
static void
count_command (verbose_string, from_tty)
     char *verbose_string;
     int from_tty;
{
  instCount = 0;
  printf("verbose_string = 0x%x\n", verbose_string);
...
static void
step_1 (skip_subroutines, single_inst, count_string, icount)
...
Argument N means do this N times (or till program stops for another reason).");
  ...
  add_com ("count", class_run, count_command,
	   "Continue program while counting instructions, stopping at breakpoints.\n\
Argument N means only do N source lines.");
