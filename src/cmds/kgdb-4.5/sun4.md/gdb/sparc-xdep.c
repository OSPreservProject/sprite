...
#define gregs ((Mach_RegState *)core_reg_sect)
    ...
    *(int *)&registers[REGISTER_BYTE (0)] = 0;
    ...
    bcopy ((gregs->globals)+(sizeof(unsigned int)),
	   &registers[REGISTER_BYTE (G1_REGNUM)],
	   7 * REGISTER_RAW_SIZE (G1_REGNUM));
    *(int *)&registers[REGISTER_BYTE (PS_REGNUM)] = gregs->curPsr;
    *(int *)&registers[REGISTER_BYTE (PC_REGNUM)] = gregs->pc;
    *(int *)&registers[REGISTER_BYTE (NPC_REGNUM)] = gregs->nextPc;
    *(int *)&registers[REGISTER_BYTE (Y_REGNUM)] = gregs->y;
...
#define fpuregs  ((Mach_RegState *) core_reg_sect)
    if (core_reg_size >= sizeof (Mach_RegState))
      {
	bcopy (fpuregs->fregs,
	       &registers[REGISTER_BYTE (FP0_REGNUM)],
	       sizeof (fpuregs->fregs));
	bcopy (&fpuregs->fsr,
	       &registers[REGISTER_BYTE (FPS_REGNUM)],
	       sizeof (FPU_FSR_TYPE));
      }
