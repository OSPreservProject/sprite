	...
	char	**argv;
{
	...
	if (targetMachine == NULL) {
	    /*
	     * If no target machine was specified on the command line,
	     * then default to the current machine type.
	     */
#ifdef sun3
	    targetMachine = "sun3";
#endif
...
#endif
#ifdef ds3100
	    targetMachine = "ds3100";
#endif
	}
	if (strcmp(targetMachine, "sun3") == 0) {
	    AddCppArg("-Dsun");
	    AddCppArg("-Dsun3");
	} else if (strcmp(targetMachine,"sun4") == 0) {
	    AddCppArg("-Dsun");
	    AddCppArg("-Dsun4");
	} else if (strcmp(targetMachine,"sun4c") == 0) {
	    AddCppArg("-Dsun");
	    AddCppArg("-Dsun4");
	    AddCppArg("-Dsun4c");
	} else if (strcmp(targetMachine, "ds3100") == 0) {
	    AddCppArg("-Dds3100");
	    AddCppArg("-Dmips");
	} else if (strcmp(targetMachine, "ds5000") == 0) {
	    AddCppArg("-Dds5000");
	    AddCppArg("-Dmips");
	} else {
	    fprintf(stderr, "Unknown target machine: %s\n", targetMachine);
	    exit(1);
	}
#endif
	...
	for(argc--, argv++; argc; argc--, argv++) {
		...
		if (argv[0][1] == 'D') {
		   ...
		   show = FALSE;
		    ...
		    if (argv[0][2])
			targetMachine = argv[0]+2;
		    else {
			argc--, argv++;
			if (! argc)
			    LogFatal("No targetMachine arg after -m flag\n", "");
			targetMachine = argv[0];
		    }
#endif		    
		} else if (argv[0][1] == 'v') {
		    verbose = TRUE;
