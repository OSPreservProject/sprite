...
#ifndef MYBUFSIZE
#if 0
/* For buffering of the files, to speed up fread. */
#define MYBUFSIZE 65536
(char *fileBuffers)[BFD_CACHE_MAX_OPEN];
#endif
#endif
      ...
      bfd *abfd)
{
  ...
  if (abfd->iostream) {
#if 0
    fileBuffers[open_files] = (char *)malloc(MYBUFSIZE);
    setbuffer(abfd->iostream, fileBuffers[open_files], MYBUFSIZE);
#endif
    open_files++;
    bfd_cache_init (abfd);
  }
