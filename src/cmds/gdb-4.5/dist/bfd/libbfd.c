...
int DEFUN(real_read,(where, a,b, file),
          ...
          FILE *file)
{
  /* The original code called fread(where, a, b, file), causing */
  /* millions of calls to bcopy to get individual chars, instead of */
  /* one call to get millions of chars. */
  if (read(fileno(file), where, b*a)) {
    return b;
  }
  return 0;
}
