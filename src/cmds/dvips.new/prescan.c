...
#endif  /* DEBUG */
      pagenum = signedquad() ;
      if (pagenum == firstpage && notfirst)
         firstmatch++ ;
      if (pagenum == lastpage && notlast)
         lastmatch++ ;
      if (notfirst && (pagenum != firstpage || firstmatch != firstseq))
         skippage() ;
      else {
         if (notlast && pagenum == lastpage)
            lastmatch-- ;
         break ;
      }
   }
 ...
 */
      while (maxpages>0 && (ret=scanpage())) {
         ...
         if (pagenum == lastpage && notlast)
            lastmatch++ ;
         if (notlast && pagenum == lastpage && lastmatch == lastseq)
