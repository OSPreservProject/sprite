    ...
    char *str;
{
	...
	special_case = 0;
	assert(the_insn.reloc == RELOC_BASE13);

	if (the_insn.exp.X_add_symbol != NULL ||
	    the_insn.exp.X_subtract_symbol != NULL ||
	    ((the_insn.exp.X_add_number & 0xffffe000) !=
	      ((the_insn.exp.X_add_number & 0x1000) ? 0xffffe000 : 0))) {

	    toP = frag_more(4);
	    rsd = (the_insn.opcode >> 25) & 0x1f;
	    assert(the_insn.opcode == (0x80102000 | (rsd << 25)));
	    the_insn.opcode = 0x01000000 | (rsd << 25);
	    ...
	    if (the_insn.exp.X_add_symbol == NULL &&
	           the_insn.exp.X_subtract_symbol == NULL &&
		   ((the_insn.exp.X_add_number & 0x000003ff) == 0)) {
	        return;
	    }
