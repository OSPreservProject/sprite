    ...
    char *str;
{
    ...
    /* put out the opcode */
    if (special_case != 0) {
	assert(special_case == SPECIAL_CASE_SET):
	special_case = 0;
	assert(the_insn.reloc == RELOC_HI22);
	/*
	 * We can have a number of cases here.
	 * 1.  Only high bits are set.  Use RELOC_HI22
	 * 2.  Only low bits are set.  Use RELOC_LO10
	 * 3.  Both high and low bits are set.  Generate 2 instructions.
	 * 4.  We might not be able to tell.  Generate 2 insns just to be safe.
	 */
	if (the_insn.X_add_symbol == NULL
	    && the_insn.X_subtract_symbol == NULL) {
	    if ((the_insn.X_add_number & 0x3ff) == 0) {
		/* only high bits are set */
		goto foobar;
	    } else if ((the_insn.X_add_number & 0xfffffc00) == 0) {
		/* only low bits are set */
	...
	/*
	 * Either both high and low bits are set, or there are unresolved
	 * symbols so that it cannot be determined in one pass.
	 */
	toP = frag_more(4);
...
md_number_to_imm(buf,val,n, fixP, seg_type)
    ...
    int seg_type;
{
    if (seg_type != N_TEXT || fixP->fx_r_type == (int) NO_RELOC) {
	switch (n) {
	case 1:
		*buf = val;
		break;
     ...
     * handle this.
     */
    if (fixP->fx_r_type == (int) RELOC_WDISP30 && fixP->fx_addsy) {
    ...
    struct reloc_info_sparc *ri_p, ri;
{
    ...
    p[1] = n >> 16;
    p[0] = n >> 24;
    n = (ri_p->r_index << 8) + (ri_p->r_extern << 7) + ((int) ri_p->r_type);
    p[7] = n;
    p[6] = n >> 8;
    p[5] = n >> 16;
    p[4] = n >> 24;
    ...
    relax_addressT segment_address_in_file;
{
	...
	if (fixP->fx_r_type >= (int) NO_RELOC) {
	...
	if ((symbolP = fixP->fx_addsy) != NULL) {
	    ...
	    ri.r_type = (enum reloc_type) fixP->fx_r_type;
	    if (fixP->fx_pcrel) {
