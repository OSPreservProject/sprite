...
#
# $Header: /user6/ouster/tcl/scripts/RCS/init.tcl,v 1.4 92/03/07 14:32:28 ouster Exp $ SPRITE (Berkeley)
#
...
proc unknown args {
	...
	if {[llength $cmds] != 0} {
		...
		error "ambiguous command name \"$name\": $cmds"
	    }
	}
    }
    unset unknown_active
...
proc auto_load cmd {
    global auto_index auto_path env

    if [info exists auto_index($cmd)] {
	source $auto_index($cmd)
	return 1
    }
