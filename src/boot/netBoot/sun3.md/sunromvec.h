 ...
 *
 * These TABLEs are the ONLY knowledge the outside world has of this PROM.
...
#if defined(SUN4) || (sun4)
#ifdef CACHE
#define romp ((struct sunromvec *) 0x00001000) /* Used when running the      *
                                                * firmware out of the cache. */
#else CACHE
#define romp ((struct sunromvec *) 0xFFE81000) /* Used when running the  *
                                                * firmware out of ROM.   */
#endif CACHE
#else defined(SUN4) || (sun4)
/*
 ...
 * 8 megabytes will be mapped beginning with location "0x0".  When a 
