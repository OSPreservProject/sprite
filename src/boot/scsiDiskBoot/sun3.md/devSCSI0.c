...
struct Controller {
    ...
    Device  *devicePtr; /* Pointers to the device
                               * attached to the
			       * controller index by. */
    ...
    ScsiCmd	*scsiCmdPtr;		/* Command to send. */
{
	...
	Mach_MonPrintf("scsi cmd %d/%d size %d addr 0x%x dma addr 0x%x\n",
		scsiCmdPtr->commandBlock[0],
		scsiCmdPtr->commandBlockLen,size, scsiCmdPtr->buffer, addr);
   /*
...
status = Wait(ctrlPtr, SCSI_INTERRUPT_REQUEST, TRUE);
    if (status == SUCCESS) {
		unsigned char statusByte;
		int	residual;
	    residual = -regsPtr->dmaCount -1;
	    status = GetStatusByte(ctrlPtr,&statusByte);
	    RequestDone(devPtr, scsiCmdPtr, status, statusByte, size-residual);
    }
    return status;
}

    ...
    int		amountTransferred; /* Amount transferred by command. */
{
    ...
    Mach_MonPrintf("Request done %d bytes status 0x%x scsi 0x%x addr 0x%x/%d\n",
		amountTransferred, status, scsiStatusByte, 
		scsiCmdPtr->buffer, scsiCmdPtr->bufferLen);
    /*
    ...
    ClientData	clientDataArg;
{
	...
	if (regsPtr->control & SCSI_BUS_ERROR) {
	    ...
	    if (!IS_CTRL_BUSY(ctrlPtr)) {
		    Mach_MonPrintf(" SCSI no command\n");
		...
		if (!ctrlPtr->scsiCmdPtr->dataToDevice) {
		  *(volatile char *)(VMMACH_DMA_START_ADDR + regsPtr->dmaAddress) =
			regsPtr->data;
		    residual--;
		} else {
		    residual++;
    ...
    DevConfigController	*ctrlLocPtr;	/* Controller location. */
{
    ...
    Controllers[0] = ctrlPtr;
    boot_Poll = nullp;
    boot_SendSCSICommand = entryAvailProc;
    Reset(ctrlPtr);
    ...
    void	(*insertProc)(); /* Queue insert procedure. */
{
    ...
    ctrlPtr->devicePtr = devPtr;
    devPtr->targetID = targetID;
    devPtr->ctrlPtr = ctrlPtr;

    devPtr->handle.locationName = "SCSI0";
