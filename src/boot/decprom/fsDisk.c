...
#ifdef notdef
static char rcsid[] = "$Header: /sprite/src/boot/decprom/RCS/fsDisk.c,v 1.2 91/08/30 17:36:16 shirriff Exp $ SPRITE (Berkeley)";
#endif not lint
...
#ifndef NO_PRINTF
    Mach_MonPrintf("Jumping to 0x%x (%d)\n", devFsOpTable[devType].open,
	devType);
#endif
...
#ifndef NO_PRINTF
	Mach_MonPrintf("Bad magic <%x>\n",
		((Ofs_DomainHeader *)buffer)->magic);
#endif
	return(FAILURE);
    }

    fsDomainPtr->headerPtr = (Ofs_DomainHeader *) buffer;
