    ...
    ClientPtr client;
{
	...
	if (PdevIsPdevConn(fd)) {
	    result = PdevRead(fd, oci->buffer + oci->bufcnt, 
		      oci->size - oci->bufcnt); 
	} else
#endif
...
#define InsertIOV(pointer, length) \
	...
	if (PdevIsPdevConn(connection)) {
	    len = PdevWritev(connection, iov, i);
	} else { 
	    len = writev(connection, iov, i);
	}
	if (len >= 0)
#else
 ...
 */
PdevClearWriteBlockHack(fd)
    int fd;
{
    if (AnyClientsWriteBlocked && GETBIT(ClientsWriteBlocked,fd))
    {
	NewOutputPending = TRUE;
	BITSET(OutputPending, fd);
