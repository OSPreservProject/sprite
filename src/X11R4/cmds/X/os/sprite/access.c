    ...
    ClientPtr client;
{
    ...
    if (PdevIsPdevConn(((OsCommPtr)client->osPrivate)->fd))
	return TRUE;
#endif
    alen = sizeof (from);
...
#ifdef SPRITEPDEVCONN
/* Check whether this is an allowed pdev connection.
  This is called from ClientAuthorized() in connection.c.
  We have to do this here because result will depend on LocalHostEnabled. */
InvalidPdevConn (fd)
	int	fd;		/* connection */
{
    if (!PdevIsPdevConn(fd))
	return 1;		/* no pdev, no cigar */
    else
	/*
	 * Sprite is like one big host for authorization
	 * purposes, so we allow pdev connections (from any
	 * Sprite host) iff LocalHostEnabled.
	 */
