...
#else	_MAP_NEW
    printf("Before mmap in Xsun\n");
    if (fbType.fb_width == CG3A_WIDTH) {
#ifdef sprite
	CG3ACfb = (CG3ACPtr) malloc((CG3AC_MONOLEN + 
	    CG3AC_ENBLEN + CG3AC_IMAGELEN + VMMACH_SEG_SIZE);
#else
...
#ifdef sprite
	CG3ACfb = (CG3ACPtr) mmap(CG3ACfb, CG3AC_MONOLEN + 
	    CG3AC_ENBLEN + CG3AC_IMAGELEN,
	    PROT_READ | PROT_WRITE,
	    MAP_SHARED, fd, 0);
	if (CG3ACfb == (Address) NULL || CG3ACfb == (Address) NIL) {
#else
...
#ifdef sprite
	CG3BCfb = (CG3BCPtr) malloc(CG3BC_MONOLEN + 
	    CG3BC_ENBLEN + CG3BC_IMAGELEN + VMMACH_SEG_SIZE);
#else
