 ...
 * rand.c --
 *
 *	Contains the source code for the "random", "srandom", "rand",
 *	and "srand" library procedures.  Taken from BSD.
 *
 * Copyright 1983, 1988 Regents of the University of California
...
#ifndef lint
static char rcsid[] = "$Header: /r3/kupfer/spriteserver/src/lib/c/stdlib/RCS/rand.c,v 1.3 91/12/12 21:51:17 kupfer Exp $ SPRITE (Berkeley)";
#endif not lint
 ...
 * Note that this initialization takes advantage of the fact that srand()
 ...
 * the best (largest) one we can and set things up for it.  srand() is
 * then called to initialize the state information.
 * Note that on return from srand(), we set state[-1] to be the type
    ...
    int			n;			/* # bytes of state info */
{
	...
	end_ptr = &state[ rand_deg ];	/* must set end_ptr before srand */
	srandom( (int) seed );
	if(  rand_type  ==  TYPE_0  )  state[ -1 ] = rand_type;
	else  state[ -1 ] = MAX_TYPES*(rptr - state) + rand_type;
	return( ostate );
...
/*
 * For compatibility with the old UNIX rand and srand:
 */
