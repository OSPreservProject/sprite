...
LOCAL INT       contcnt;    /* Number of loop levels to continue */
...
*/
		...
		case TFORK:
			...
			THEN	/* This is the parent branch of fork;    */
...
#ifdef sprite
				THEN	xtos(parent); prs(numbuf); newline();
#else /* sprite */
				...
				THEN	post(parent);
#ifdef sprite
				ELSE    xtos(parent); replace(&pcsadr, numbuf);
#else /* sprite */
