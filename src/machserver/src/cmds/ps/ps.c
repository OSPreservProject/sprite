...
#ifndef lint
static char rcsid[] = "$Header: /user5/kupfer/spriteserver/src/cmds/ps/RCS/ps.c,v 1.3 92/04/24 00:00:11 kupfer Exp $ SPRITE (Berkeley)";
#endif not lint
...
#else
    /* 
     * If I have to express a time using a single integer, it will probably 
     * be in milliseconds.  However, any non-zero number will probably do 
     * here. 
     */
				 ...
				 * process. */
{
	...
	status = Vm_Cmd(VM_GET_STATS, sizeof(vmStat), &vmStat);
	if (status != SUCCESS) {
				 ...
				 * process. */
{
    ...
    if (firstTime) {
	firstTime = 0;
	printf("PID     MID PPID  GROUP USER     RUSER      TIME COMMAND\n");
    }
    ...
    printf("%5x %5d %5x %s %-8.8s %-8.8s%7s %s\n",
	    infoPtr->processID, infoPtr->machID, infoPtr->parentID, family,
	    UserString(infoPtr->effectiveUserID), 
