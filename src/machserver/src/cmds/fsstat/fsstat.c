...
#ifndef lint
static char rcsid[] = "$Header: /user5/kupfer/spriteserver/src/cmds/fsstat/RCS/fsstat.c,v 1.5 92/07/13 12:58:18 kupfer Exp $ SPRITE (Berkeley)";
#endif not lint
...
Option optionArray[] = {
    ...
    {OPT_TRUE, "H", (Address)&printHostInfo, "Print Sprite version and uptime info"},
...
/* 
 * These counts hold sums of vmstat counts.  They mostly exist to keep the 
 * summer routine from vmstat happy.
 */
int totalPagesRead;		/* unused */
int totalPartialPagesRead;	/* unused */
int totalPagesZeroed;		/* unused */
int totalPagesWritten;
int totalPagesCleaned;		/* unused */
    ...
    char *argv[];
{
    ...
    status = Vm_Cmd(VM_GET_STATS, sizeof(vmStats), &vmStats);
    if (status != SUCCESS) {
	Stat_PrintMsg(status, "Vm_Cmd failed");
	exit(status);
    }
    Vm_PageSize(&pageSize);
    TotalPageCounts(&vmStats, &totalPagesRead, &totalPartialPagesRead,
		    &totalPagesZeroed, &totalPagesWritten, &totalPagesCleaned);
    vmBlocksWritten = totalPagesWritten * (pageSize / FS_BLOCK_SIZE);
...
#define NUM_BYTE_COUNT_COLUMNS 4

  {
      ...
      printf("HANDLES %d (%d) limbo %d scav %d (dirs %d)\n",
	    fsStats.handle.exists,
	    fsStats.handle.maxNumber,
	    fsStats.handle.limbo,
	    fsStats.handle.lruHits,
	    fsStats.object.dirFlushed);
      printf("\tlooks %d scans %d looks/scan %.2f\n",
	     fsStats.handle.lruChecks, fsStats.handle.lruScans,
