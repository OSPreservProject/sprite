 ...
 *	Job_RestartJobs		Try to restart any stopped jobs if there are
 *				slots available.
 */
#ifndef lint
static char     *rcsid = "$Id: job.c,v 1.2 92/05/12 12:19:06 kupfer Exp $ SPRITE (Berkeley)";
#endif lint
 ...
 * been migrted home, the job is placed on the stoppedJobs queue to be run
...
/* 
 * This is the error format to use in case KILL fails.  The first field is 
 * the name of the function, the second is the signal number, the third is 
 * the process ID, and the fourth is the reason why KILL failed.
 */
static char killFailedMsg[] = 
#ifdef sprite
    "%s: couldn't send signal %d to process %x: %s\n";
#else
    "%s: couldn't send signal %d to process %d: %s\n";
#endif
    ...
    int	    	signo;	    /* Signal to send it */
{
	...
	if (KILL(job->pid, signo) < 0 && DEBUG(JOB)) {
	    printf(killFailedMsg, "JobCondPassSig", signo, job->pid,
		   strerror(errno));
	    fflush(stdout);
	}
    }
    ...
    if (DEBUG(JOB)) {
#ifdef sprite
	printf("JobCondPassSig passing signal %d to child %x.\n", signo,
	       job->pid);
#else /* sprite */
	printf("JobCondPassSig passing signal %d to child %d.\n", signo,
	       job->pid);
#endif /* sprite */
    }
    if (KILL(job->pid, signo) < 0 && DEBUG(JOB)) {
    ...
    int	    signo;	/* The signal number we've received */
{
     ...
     */
    mask = sigblock(0);
    if (sigsetmask(~0 & ~(1 << (signo-1))) < 0) {
	if (DEBUG(JOB)) {
	    perror("sigsetmask");
	}
    }
    if (DEBUG(JOB)) {
	printf("JobPassSig passing signal to self, mask = %x.\n",
	       ~0 & ~(1 << (signo-1)));
    }
    if ((int) signal(signo, SIG_DFL) < 0) {
	if (DEBUG(JOB)) {
	    perror("signal");
	}
    }
 ...
 *	are equal, if the job is remote. 
 *
 * Results:
...
JobCmpRmtID (job, rmtID)
    Job            *job;	/* job to examine */
    int             rmtID;	/* remote id desired */
{
    return (!((job->flags & JOB_REMOTE) && (rmtID == job->rmtID)));
}

/*-
 *-----------------------------------------------------------------------
...
#define DBPRINTF(fmt, arg) if (DEBUG(JOB)) {printf("+++ "); printf (fmt, arg);} fprintf (job->cmdFILE, fmt, arg)
    ...
    union wait	  status;     	  /* sub-why job went away */
{
	 ...
	 * status...
	 *
	 * And one more thing: send the process group a continue signal
	 * in case the parent has exited (that's why we're here) but
	 * we haven't continued the rest of the group.  Else we might
	 * never get the EOF on the pipe since there's still a process
	 * out there someplace with the other end of the pipe open.
	 */
	if (KILL(job->pid, SIGCONT) < 0) {
	    if (DEBUG(JOB)) {
		printf(killFailedMsg, "JobFinish", SIGCONT, job->pid,
		       strerror(errno));
		fflush(stdout);
	    }
	}
	...
	if (usePipes) {
	    ...
	    /*
	     * Don't wait for the pipe to be closed.  A child might still
	     * have it open.
	     */
	...
	if (WIFEXITED(status)) {
		...
		if (DEBUG(JOB)) {
#ifdef sprite
		    printf("Warning: process %x was not continuing.\n", job->pid);
#else
		    printf("Warning: process %d was not continuing.\n", job->pid);
#endif				/* sprite */
		}
#ifdef notdef
		/*
		...
		if (DEBUG(JOB)) {
#ifdef sprite
		    printf("Process %x is continuing locally.\n", job->pid);
#else
		    printf("Process %d is continuing locally.\n", job->pid);
#endif				/* sprite */
		}
		nLocal += 1;
	    }
    ...
    char    	  **argv;
{
	...
	if ((job->node->type & OP_BACKGROUND) || background) {
#ifdef sprite
	    ReturnStatus status;

	    status = Proc_SetPriority(PROC_MY_PID, PROC_LOW_PRIORITY, FALSE);
	    if (status != SUCCESS) {
		fprintf(stderr, "Warning: error setting priority: %s\n",
			Stat_GetMsg(status));
	    }
#else /* sprite */
	    /* I don't know the UNIX call offhand. */
#endif /* sprite */	    
	}
#endif /* SPRITED_USERPRIORITY */
    ...
    Job 	  *job;    	/* Job to restart */
{
	...
	if (!Rmt_ReExport(job->pid, job->node, &host)) {
		...
		if (KILL(job->pid, SIGCONT) < 0 && DEBUG(JOB)) {
		    printf(killFailedMsg, "JobRestart", SIGCONT, job->pid,
			   strerror(errno));
		    fflush(stdout);
		}
		nLocal +=1;
    ...
    Boolean	  block;    	/* TRUE if should block on the wait. */
{
    ...
    if ((pid = wait3(&status, (block?0:WNOHANG)|WUNTRACED,
			(struct rusage *)0)) > 0)
    {
	if (DEBUG(JOB)) {
#ifdef sprite
	    printf("Process %x exited, stopped, or continued.\n", pid);
#else
	...
	if (jnode == NILLNODE) {
		...
		if (jnode == NILLNODE) {
#ifdef sprite
		    Error("Resumed child (%x) not in table of stopped jobs",
			  pid);
#else
	    ...
	    if (!(job->flags & JOB_REMOTE)) {
		if (DEBUG(JOB)) {
		    printf("Job queue has one fewer local process.\n");
		}
		nLocal -= 1;
	    }
	}
...
Job_CatchOutput ()
{
	...
	if ((nfds = select (FD_SETSIZE, &readfds, (int *) 0, (int *) 0, &timeout)) < 0)
	{
	    ...
	    if ((mig_GlobalPdev != -1) &&
		(FD_ISSET(mig_GlobalPdev, &readfds))) {
		/*
		 * Migration pdev is selectable.  Find out what the daemon
		 * wants from us.
		 */
		(void) Mig_ConfirmIdle(0);
		nfds -= 1;
	    }
				 ...
				 * target should be executed */
{
	...
	if (job->flags & JOB_REMOTE) {
	    ...
	    if (KILL(job->pid, SIGINT) < 0 && DEBUG(JOB)) {
		printf(killFailedMsg, "JobInterrupt", SIGINT, job->pid,
		       strerror(errno));
		fflush(stdout);
	    }
	}
	...
	if (job->pid) {
	    if (DEBUG(JOB)) {
#ifdef sprite
		printf("JobInterrupt passing signal to child %x.\n",
		       job->pid);
#else /* sprite */
		printf("JobInterrupt passing signal to child %d.\n", 
		       job->pid);
#endif /* sprite */
	    }
	    if (KILL(job->pid, SIGINT) < 0 && DEBUG(JOB)) {
	...
	if (job->flags & JOB_RESTART) {
	    if (DEBUG(JOB)) {
		printf("JobInterrupt skipping job on stopped queue -- it was waiting to be restarted.\n");
	    }
	    continue;
	}
	if (!Targ_Precious (job->node)) {
	...
	if (DEBUG(JOB)) {
#ifdef sprite
	    printf("JobInterrupt passing CONT to stopped child %x.\n",
		   job->pid);
#else /* sprite */
	    printf("JobInterrupt passing CONT to stopped child %d.\n", 
		   job->pid);
#endif /* sprite */
	}
	if (KILL(job->pid, SIGCONT) < 0 && DEBUG(JOB)) {
	    printf(killFailedMsg, "JobInterrupt", SIGCONT, job->pid,
		   strerror(errno));
	    fflush(stdout);
	}
#ifdef RMT_WANTS_SIGNALS
	if (job->flags & JOB_REMOTE) {
	    ...
	    if (DEBUG(JOB)) {
#ifdef sprite
		printf("JobInterrupt passing interrupt to stopped child %x.\n",
		       job->pid);
#else /* sprite */
		printf("JobInterrupt passing interrupt to stopped child %d.\n", 
		       job->pid);
#endif /* sprite */
	    }
	    if (KILL(job->pid, SIGINT) < 0 && DEBUG(JOB)) {
...
Job_AbortAll ()
{
	...
	while ((ln = Lst_Next (jobs)) != NILLNODE) {
		...
		if (KILL(job->pid, SIGINT) < 0 && DEBUG(JOB)) {
		    printf(killFailedMsg, "Job_AbortAll", SIGINT,
			   job->pid, strerror(errno));
		    fflush(stdout);
		}
		if (KILL(job->pid, SIGKILL) < 0 && DEBUG(JOB)) {
		    printf(killFailedMsg, "Job_AbortAll", SIGKILL,
			   job->pid, strerror(errno));
		    fflush(stdout);
		}
	    }
#else
	    if (KILL(job->pid, SIGINT && DEBUG(JOB)) < 0) {
		printf(killFailedMsg, "Job_AbortAll", SIGINT,
		       job->pid, strerror(errno));
		fflush(stdout);
	    }
	    if (KILL(job->pid, SIGKILL && DEBUG(JOB)) < 0) {
		printf(killFailedMsg, "Job_AbortAll", SIGKILL,
		       job->pid, strerror(errno));
		fflush(stdout);
	    }
    ...
    int 	  hostID;    	/* ID of host we used, for matching children. */
{
    ...
    if (DEBUG(JOB)) {
	printf("JobFlagForMigration(%d) called.\n", hostID);
	fflush(stdout);
    }
    ...
    if (DEBUG(JOB)) {
	printf("JobFlagForMigration(%d) found job '%s'.\n", hostID,
	       job->node->name);
	fflush(stdout);
    }
#ifdef notdef
    if (! ((job->node->type & OP_BACKGROUND) || background)) {
	if (DEBUG(JOB)) {
	    printf("JobFlagForMigration ignoring regular priority process %x.\n",
		   job->pid);
	    fflush(stdout);
	}
	return;
    ...
    /*
     * Make sure the process is on its way home even if it wasn't explicitly
     * evicted.   (Commented out because the signal causes ultrix-compatible
     * binaries to break.)
     */
    KILL(job->pid, SIGMIGHOME);
#endif

    /*
     * Work around kernel bug that won't pass signals consistently to
     * a process that's in the middle of a migration.  Wait for eviction
     * to complete before sending signals or remigrating. 
     */
#define CHECK_FOR_MIG_IN_PROGRESS
#ifdef CHECK_FOR_MIG_IN_PROGRESS
    {
	ReturnStatus status;
#define NUM_PCBS 256
	...
	for (i = 0, infoPtr = infos; i < pcbsUsed; i++, infoPtr++) {
	    ...
	    if (infoPtr->familyID == job->pid) {
		if (DEBUG(JOB)) {
		    printf("Found process %x (family %x), state is %d\n",
			   infoPtr->processID, job->pid,
			   infoPtr->state);
		    fflush(stdout);
		}
#define MIG_IN_PROGRESS_RETRIES 6
		if (infoPtr->state == PROC_MIGRATED) {
		    Proc_PCBInfo newInfo;
		    int returned;
		    int retries = 0;

		    if (verboseRemigrates || DEBUG(JOB)) {
			printf("JobFlagForMigration: waiting for process %x to complete eviction or exit.\n",
			       infoPtr->processID);
			fflush(stdout);
		    }
		    do {
			/*
			 * Exponential backoff.  Make sure to sleep at least
			 * one second, though.
			 */
			sleep(1 + retries * 2);
			status = Proc_GetPCBInfo(infoPtr->processID,
						 infoPtr->processID,
						 homeHost,
						 sizeof(Proc_PCBInfo),
						 &newInfo,
			...
			if (status != SUCCESS) {
			    if (DEBUG(JOB)) {
				fprintf(stderr,
					"JobFlagForMigration: couldn't get process table entry for pid %x: %s\n",
			...
			if (returned != 1) {
			    fprintf(stderr,
				    "JobFlagForMigration: couldn't get process table entry for pid %x: Proc_GetPCBInfo returned %d elements.\n",
				    infoPtr->processID, returned);
			    break;
			}
			retries++;
		    } while ((newInfo.state == PROC_MIGRATED) &&
			     (retries < MIG_IN_PROGRESS_RETRIES));
		    if (retries == MIG_IN_PROGRESS_RETRIES) {
			fprintf(stderr,
				"JobFlagForMigration: warning: eviction of process %x apparently did not complete.\n",
				infoPtr->processID);
			return;
		    } else if (DEBUG(JOB)) {
			printf("JobFlagForMigration: process %x completed eviction.\n",
			       infoPtr->processID);
			fflush(stdout);
		    }
		}
...
#endif /* CHECK_FOR_MIG_IN_PROGRESS */

    if (KILL(job->pid, SIGSTOP) < 0 && DEBUG(JOB)) {
	printf(killFailedMsg, "JobFlagForMigration", SIGSTOP,
	       job->pid, strerror(errno));
	fflush(stdout);
    }
...
Job_RestartJobs()
{
	...
	if (DEBUG(JOB)) {
	    printf("Job queue is not full.  Restarting a stopped job.\n");
	}
