 ...
 */
#ifndef lint
static char     *rcsid = "$Id: var.c,v 1.1 92/05/11 16:27:43 kupfer Exp $ SPRITE (Berkeley)";
#endif lint
    ...
    Boolean 	    *freePtr; 	/* OUT: TRUE if caller should free result */
{
	...
	*freePtr = FALSE;
	...
	if (dynamic) {
	    if (DEBUG(VAR)) {
		/* 
		 * Pretty up the "returning" string for the user, who may 
		 * be confused by the extra byte at the start of it.
		 */
		printf("Oops, expanding local variable in global context;\n");
		printf("returning \"%s\" instead of \"%s\"\n", start+1,
		       str);
	    }
	    str = malloc(*lengthPtr + 1);
	    strncpy(str, start, *lengthPtr);
