 ...
 */
#ifndef lint
static char     *rcsid = "$Id: targ.c,v 1.2 92/05/12 12:23:53 kupfer Exp $ SPRITE (Berkeley)";
#endif lint
...
#define PRINTBIT(attr)	\
    case CONCAT(OP_,attr): printf("." #attr " "); fflush(stdout); break
#define PRINTDBIT(attr) \
    case CONCAT(OP_,attr): if (DEBUG(TARG)) { \
        printf("." #attr " "); \
        fflush(stdout); \
	} \
	break
