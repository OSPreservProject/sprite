 ...
 */
#ifndef lint
static char     *rcsid = "$Id: parse.c,v 1.5 91/05/29 15:34:32 kupfer Exp $ SPRITE (Berkeley)";
#endif lint
 ...
 * if a target or source is special. (* Therefore, the keywords MUST
 * be kept in alphabetical order! *)  The 'spec' field is the ParseSpecial
...
{ ".BACKGROUND", Background, 	OP_BACKGROUND },
{ ".BEGIN", 	  Begin,    	0 },
{ ".DEFAULT",	  Default,  	0 },
    ...
    char           *line;	/* the line to parse */
{
	...
	*cp = savec;
     ...
     *	a .BACKGROUND makes all jobs treated as long-running.
     */
    if (!*line) {
	switch (specType) {
	    case Suffixes:
