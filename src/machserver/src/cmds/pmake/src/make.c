 ...
 */
#ifndef lint
static char     *rcsid = "$Id: make.c,v 1.2 92/05/12 12:23:50 kupfer Exp $ SPRITE (Berkeley)";
#endif lint
    ...
    register GNode *gn;	      /* the node to check */
{
	...
	if (DEBUG(MAKE)) {
	    ...
	    } else if (gn->cmtime >= now) {
		printf("child modified since pmake started...");
	    } else {
			...
			/* added */
{
	...
	if (pgn->type & OP_JOIN) {
	    if (cgn->made == MADE) {
		if (DEBUG(MAKE)) {
		    printf("%s JOIN OODATE wrt %s\n", cgn->name, pgn->name);
		}
		Var_Append(OODATE, child, pgn);
	    } else if (DEBUG(MAKE)) {
		printf("%s JOIN not OODATE wrt %s\n", cgn->name, pgn->name);
	    }
		   ...
		   (cgn->mtime >= now))
	{
	     ...
	     * the start of the make and we think we had something to do with
	     * the child being modified. This is to keep pmake from getting
	     ...
	     *
	     * XXX: In fact, this will cause unmade children with later
	     * timestamps to go into OODATE, but again, better safe than sorry.
	     * Otherwise clock skew kills us.
	     */
	    if (DEBUG(MAKE)) {
		printf("%s OODATE wrt %s\n", cgn->name, pgn->name);
	    }
	    Var_Append(OODATE, child, pgn);
	} else if (DEBUG(MAKE)) {
	    printf("%s not OODATE wrt %s (pgn %d cgn %d now %d)\n",
		   cgn->name, pgn->name, pgn->mtime, cgn->mtime, now);
	}
    }
    return (0);
}
