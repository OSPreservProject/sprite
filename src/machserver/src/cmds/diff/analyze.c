     ...
     int *cost;
{
  ...
  int *const xv = xvec;		/* Still more help for the comiler. */
     ...
     struct file_data filevec[];
{
  ...
  /* Don't really discard the provisional lines if there are less than ten
     discardable lines in a row.  */
	...
	if (discards[i])
	  {
	    ...
	    /* Cancel provisional discards at the beginning.  */
	    while (i < end && discards[i] == 2)
	      discards[i] = 0;
	    ...
	    /* If half the lines in the run are provisional,
	       cancel discarding of all provisional lines in the run.  */
	    if (provisional * 2 > length)
	      {
		while (j > i)
		  if (discards[--j] == 2)
		    discards[j] = 0;
	      ...
	      {
		/* Cancel provisional discards that are near either end.  */
		for (j = 0; j < 5 && j < length; j++)
		  if (discards[i + j] == 2)
		    discards[i + j] = 0;
		/* Meanwhile, I advances to the last line of the run.  */
		i += length - 1;
		length -= 5;
		for (j = 0; j < 5 && j < length; j++)
		  if (discards[i - j] == 2)
