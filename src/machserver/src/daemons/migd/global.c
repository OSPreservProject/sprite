    ...
    Host_Entry *hostPtr;
{
    ...
    /*
     * Hack alert!  Make the ds3100 and the ds5000 the same machine type,
     * but make a ds5000 worth more than a ds3100 so they will be favored.
     * JHH 3/4/91
     */
    if (!strcmp(hostPtr->machType, "ds3100")) {
	machType = "ds";
	value = 1;
    } else if (!strcmp(hostPtr->machType, "ds5000")) {
	machType = "ds";
	value = 2;
    } else {
	machType = hostPtr->machType;
	value = 1;
    }
    for (i = 0; i < maxArchTypes; i++) {
	if (archTypesArray[i] == (char *) NULL) {
	    archTypesArray[i] = Malloc(strlen(machType) + 1);
	    strcpy(archTypesArray[i], machType);
	    archType = i;
	    break;
	}
	if (strcmp(machType, archTypesArray[i]) == 0) {
	    archType = i;
	    break;
	}
    }
	...
	archTypesArray[maxArchTypes] =
	    Malloc(strlen(machType) + 1);
	strcpy(archTypesArray[maxArchTypes], machType);
	...
	*outBufSizePtr < (1 + numHosts) * sizeof(int) ||
	...
	while (total < numHosts && tryPrio <= priority) {
		...
		if (infoPtr->foreign[priority] >= infoPtr->maxProcs) {
			...
			headerPtr = (List_Links *) 
				    &idleHostsArray[priority + 1][archType];
			compPtr = (Migd_Info *) List_Last(headerPtr);
			while (!List_IsAtEnd(headerPtr, 
				(List_Links *) compPtr)) {
			    if (compPtr->value >= migdPtr->value) {
				List_Insert(migdPtr, compPtr);
				break;
			    }
			    compPtr = (Migd_Info *) 
				List_Prev((List_Links *) compPtr);
			}
			if (List_IsAtEnd(headerPtr, (List_Links *) compPtr)) {
    ...
    Migd_Info *migdPtr;
{
	...
	if (infoPtr->foreign[priority] == infoPtr->maxProcs) {
		...
		headerPtr = (List_Links *) 
			&idleHostsArray[priority + 1][archType];
		compPtr = (Migd_Info *) List_Last(headerPtr);
		while (!List_IsAtEnd(headerPtr, (List_Links *) compPtr)) {
		    if (compPtr->value >= migdPtr->value) {
			List_Insert(migdPtr, compPtr);
			break;
		    }
		    compPtr = (Migd_Info *) List_Prev((List_Links *) compPtr);
		}
		if (List_IsAtEnd(headerPtr, (List_Links *) compPtr)) {
		    List_Insert((List_Links *) migdPtr,LIST_ATFRONT(headerPtr));
		}
		goto done;
	    }
	}
    }
    headerPtr = (List_Links *) &idleHostsArray[MIG_LOW_PRIORITY][archType];
    compPtr = (Migd_Info *) List_Last(headerPtr);
    while (!List_IsAtEnd(headerPtr, (List_Links *) compPtr)) {
	if (compPtr->value >= migdPtr->value) {
	    List_Insert(migdPtr, compPtr);
